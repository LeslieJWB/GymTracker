...
adviceRouter.post("/advice/daily-summary", async (req, res) => {
    ...
    try {
      ...
      `, [appUser.id, date, DAILY_SUMMARY_PAST_WEIGHT_LIMIT]);
        const todayExerciseText = todayExerciseLines.length > 0
            ? todayExerciseLines.join("\n")
            : "No completed exercises logged yet for today.";
        const pastExerciseText = exerciseItemIds.length > 0
            ? exerciseItemIds
                .map((exerciseItemId) => {
                const lines = pastExerciseByItem.get(exerciseItemId);
                const exerciseName = todayCompletedResult.rows.find((row) => row.exercise_item_id === exerciseItemId)?.exercise_name ??
                    "Exercise";
