...
function normalizeAdviceNote(value) {
    if (!value) {
        return null;
    }
...
function groupedExerciseSetLines(rows) {
    const linesByExercise = new Map();
...
function formatAdviceDuration(seconds) {
...
function formatCardioSessionLine(row) {
    ...
    const distanceSuffix = row.distance_km !== null && row.distance_km.trim().length > 0
...
function groupedCardioSessionLines(rows) {
    const linesByExercise = new Map();
    for (const row of rows) {
        if (!linesByExercise.has(row.exercise_id)) {
            linesByExercise.set(row.exercise_id, []);
...
function formatPastExerciseHistoryByItem(pastRows) {
    ...
    const pastExerciseByItem = new Map();
    for (const [exerciseItemId, perDate] of byItem) {
        const lines = [];
        for (const [recordDate, session] of perDate) {
            lines.push(`${recordDate} | exercise note: ${session.exerciseNotes ?? "none"} | ${session.sets.join(", ")}`);
        }
        pastExerciseByItem.set(exerciseItemId, lines);
    }
    return pastExerciseByItem;
}
function formatPastCardioHistoryByItem(pastRows) {
    const byItem = new Map();
    for (const row of pastRows) {
        if (!byItem.has(row.exercise_item_id)) {
            byItem.set(row.exercise_item_id, new Map());
    ...
    for (const [exerciseItemId, perDate] of byItem) {
        const lines = [];
        for (const [recordDate, session] of perDate) {
            lines.push(`${recordDate} | exercise note: ${session.exerciseNotes ?? "none"} | ${session.sessions.join(", ")}`);
        }
        pastExerciseByItem.set(exerciseItemId, lines);
    }
    return pastExerciseByItem;
}
function formatPastExerciseText(exerciseItemIds, todayRows, pastExerciseByItem, emptyMessage) {
    ...
    return exerciseItemIds
        ...
        const exerciseName = todayRows.find((row) => row.exercise_item_id === exerciseItemId)?.exercise_name ?? "Exercise";
        return `${exerciseName}:\n${lines && lines.length > 0 ? lines.join("\n") : "No prior completed records."}`;
    })
        .join("\n\n");
}
function formatNowContext() {
    const now = new Date();
...
function themeContextBlock(theme) {
    if (!theme) {
        return "Today's theme: not set";
    }
    return `Today's theme: ${theme}`;
}
async function getTodayTheme(userId, date) {
...
function fallbackNutritionTargets(weightKg) {
...
function parseDailyNutritionTargets(raw) {
    const cleaned = raw.replace(/^```json\s*/i, "").replace(/\s*```$/i, "").trim();
    if (!cleaned) {
        return null;
    }
...
adviceRouter.post("/advice/daily-summary", async (req, res) => {
    ...
    const fallback = (review) => res.json({
        source: "fallback",
        review
    });
    try {
        ...
        const todayTheme = await getTodayTheme(appUser.id, date);
        const themeContext = themeContextBlock(todayTheme);
        const todayStrengthResult = await pool.query(`
        SELECT
          e.id AS exercise_id,
          e.exercise_item_id,
          ei.name AS exercise_name,
      ...
      `, [appUser.id, date, CARDIO_CATEGORY]);
        const todayCardioResult = await pool.query(`
        SELECT
          e.id AS exercise_id,
          e.exercise_item_id,
          ei.name AS exercise_name,
      ...
      `, [appUser.id, date, CARDIO_CATEGORY]);
        ...
        const strengthExerciseItemIds = Array.from(new Set(todayStrengthResult.rows.map((row) => row.exercise_item_id)));
        const cardioExerciseItemIds = Array.from(new Set(todayCardioResult.rows.map((row) => row.exercise_item_id)));
        const pastStrengthByItem = new Map();
        ...
        const pastCardioByItem = new Map();
      ...
      `, [appUser.id, date, DAILY_SUMMARY_PAST_WEIGHT_LIMIT]);
        const todayStrengthText = todayStrengthLines.length > 0
            ? todayStrengthLines.join("\n")
            : "No completed strength exercises logged yet for today.";
        const todayCardioText = todayCardioLines.length > 0
            ? todayCardioLines.join("\n")
            : "No completed cardio logged yet for today.";
        const pastStrengthText = formatPastExerciseText(strengthExerciseItemIds, todayStrengthResult.rows, pastStrengthByItem, "No strength exercise history context (no completed strength exercise for today).");
        const pastCardioText = formatPastExerciseText(cardioExerciseItemIds, todayCardioResult.rows, pastCardioByItem, "No cardio history context (no completed cardio for today).");
