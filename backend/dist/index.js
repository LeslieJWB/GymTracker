...
async function findIdempotentResponse(userId, endpoint, key) {
    const result = await pool.query(`
...
async function saveIdempotentResponse(userId, endpoint, key, status, body) {
    await pool.query(`
...
function buildFallbackAdvice(targetDate, rows) {
    if (rows.length === 0) {
...
app.delete("/exercises/:exerciseId", async (req, res) => {
    ...
    try {
        const result = await pool.query("DELETE FROM exercises WHERE id = $1 RETURNING id", [exerciseId]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: "Exercise not found" });
        }
...
app.delete("/exercise-sets/:setId", async (req, res) => {
    ...
    try {
        const result = await pool.query("DELETE FROM exercise_sets WHERE id = $1 RETURNING id", [setId]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: "Set not found" });
        }
        return res.status(204).send();
