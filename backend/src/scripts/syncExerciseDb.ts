...
function mapRows(sourceRows: SourceExercise[]): ExerciseSeedRow[] {
...
async function fetchSourceRows(): Promise<SourceExercise[]> {
  ...
  const payload = (await response.json()) as SourceExercise[];
  return payload;
}

async function readCachedRows(): Promise<ExerciseSeedRow[] | null> {
  try {
    const content = await readFile(cacheDataPath, "utf8");
    return JSON.parse(content) as ExerciseSeedRow[];
  } catch {
    return null;
  }
}

async function writeCache(rows: ExerciseSeedRow[]): Promise<void> {
  await mkdir(path.dirname(cacheDataPath), { recursive: true });
  await writeFile(cacheDataPath, JSON.stringify(rows, null, 2), "utf8");
}

async function downloadImageIfNeeded(imagePath: string): Promise<boolean> {
...
export async function syncExerciseDb(options?: {
  ...
  imageLimit?: number;
}): Promise<{ totalRows: number; downloadedImages: number }> {
...
async function runCli(): Promise<void> {
  ...
  console.log(
    `Exercise DB synced. Rows: ${result.totalRows}. Downloaded images: ${result.downloadedImages}.`
  );
}
