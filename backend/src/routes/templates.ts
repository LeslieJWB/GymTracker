...
templatesRouter.get("/templates/:templateId", async (req, res) => {
  ...
  try {
    const appUser = await upsertUserFromAuth(req.auth);
    const templateResult = await pool.query<{ id: string; name: string; created_at: string; updated_at: string }>(
      `
    ...
    const setsByTemplateExerciseId = new Map<
      string,
      Array<{ id: string; reps: number; weight: number; setOrder: number; notes: string | null }>
    >();
  ...
  try {
    ...
    const payload = await withTransaction(async (client) => {
      ...
      const createdExercises: Array<{
        ...
        notes: string | null;
        sortOrder: number;
        sets: Array<{ id: string; reps: number; weight: number; setOrder: number; notes: string | null }>;
      }> = [];
      ...
      for (const [exerciseIndex, exercise] of exercises.entries()) {
        ...
        const createdSets: Array<{ id: string; reps: number; weight: number; setOrder: number; notes: string | null }> = [];
