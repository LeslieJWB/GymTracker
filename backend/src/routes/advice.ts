...
type RelatedExercisePlanHistoryRow = ExercisePlanHistoryRow & {
  exercise_item_id: string;
  exercise_name: string;
};
...
type SessionHistory = {
  ...
  sets: { reps: number; weight: string; notes: string | null }[];
};

type ExercisePlanHistoryContext =
  | { kind: "direct"; historyText: string }
  | { kind: "related"; muscleGroup: string; historyText: string }
  | { kind: "none" };

function groupRowsByDate(rows: ExercisePlanHistoryRow[], sessionsLimit: number): Map<string, SessionHistory> {
  const byDate = new Map<string, SessionHistory>();
  for (const row of rows) {
...
function formatSessionHistoryLines(byDate: Map<string, SessionHistory>): string[] {
...
async function fetchDirectExerciseHistory(
  userId: string,
  exerciseItemId: string,
  date: string
): Promise<ExercisePlanHistoryRow[]> {
  const result = await pool.query<ExercisePlanHistoryRow>(
    `
...
async function fetchRelatedMuscleGroupHistory(
  userId: string,
  exerciseItemId: string,
  date: string
): Promise<{ muscleGroup: string; rows: RelatedExercisePlanHistoryRow[] } | null> {
  const muscleGroupResult = await pool.query<{ muscle_group: string | null }>(
    `
      SELECT muscle_group
      FROM exercise_items
      WHERE id = $1
  ...
  const result = await pool.query<RelatedExercisePlanHistoryRow>(
    `
      SELECT
        r.record_date::text,
        e.exercise_item_id,
...
function formatRelatedExerciseHistory(rows: RelatedExercisePlanHistoryRow[], sessionsLimit: number): string {
  const byExercise = new Map<string, { exerciseName: string; rows: RelatedExercisePlanHistoryRow[] }>();
  for (const row of rows) {
...
async function buildExercisePlanHistoryContext(
  userId: string,
  exerciseItemId: string,
  date: string
): Promise<ExercisePlanHistoryContext> {
...
function buildExercisePlanHistoryPromptSection(
  historyContext: ExercisePlanHistoryContext,
  exerciseName: string
): string {
...
function buildExerciseFeedbackPastPromptSection(
  historyContext: ExercisePlanHistoryContext,
  exerciseName: string
): string {
  if (historyContext.kind === "direct") {
...
adviceRouter.post("/advice/daily-summary", async (req, res) => {
  ...
  try {
    ...
    const todayWeightResult = await pool.query<{ weight_kg: string; body_fat_percentage: string | null }>(
      `
            ...
            .map(
              (row) =>
                `${row.created_at} | ${row.description} | ${Number(row.calories_kcal)} kcal | ${Number(
                  row.protein_g
                )} g protein | ${Number(row.fat_g)} g fat | note: ${row.llm_comment}`
            )
            .join("\n")
    ...
    const pastNutritionText =
      pastNutritionResult.rows.length > 0
        ? pastNutritionResult.rows
            .map(
              (row) =>
                `${row.record_date} | ${Number(row.total_calories_kcal)} kcal | ${Number(row.total_protein_g)} g protein | ${Number(row.total_fat_g)} g fat`
            )
            .join("\n")
        : "No prior nutrition records.";

    const formatWeightLine = (dateStr: string, weightKg: string, bodyFatPct: string | null): string => {
  ...
  try {
    ...
    if (!needsCaloriesFromLlm && !needsProteinFromLlm && !needsFatFromLlm) {
      const responsePayload = {
        ...
        comment:
          userCalorieOverride !== null || userProteinOverride !== null || userFatOverride !== null
